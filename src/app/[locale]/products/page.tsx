...
import { DigitalCatalogShowcase } from "@/components/public/DigitalCatalogShowcase";
...
export default async function ProductsPage({
  ...
  const [categoriesRaw, crossCatSetting, publishedCatalogs, autoCatalogs] =
    await Promise.all([
      ...
      prisma.digitalCatalog.findMany({
        where: { published: true, autoGenerated: false },
        include: {
          translations: { where: { locale } },
      ...
      prisma.digitalCatalog.findMany({
        where: { published: true, autoGenerated: true },
        include: {
          translations: { where: { locale } },
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#050c19_0%,#040916_100%)] px-6 pb-24 pt-32">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        {/* Auto-generated digital catalogs */}
        {autoCatalogs.length > 0 && (
          <div className="mt-12 space-y-10">
            {autoCatalogs.map((cat) => {
              const title =
                cat.translations.find((t) => t.locale === locale)?.title ||
                cat.translations[0]?.title ||
                t("autoCatalog");
              return (
                <DigitalCatalogShowcase
                  key={cat.id}
                  pages={cat.pages}
                  title={title}
                />
